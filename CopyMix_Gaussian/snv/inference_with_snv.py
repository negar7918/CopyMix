...
class Dirichlet:
    ...
    def _get_expectation_of_log(self): #protected method
...
class Beta:
    ...
    def _get_expectation_of_log(self): #protected method
        '''Just a side note:
        expectation w.r.t. Beta is digamma(alpha) - digamma(alpha+beta)
        expectation w.r.t. the complement (1-Beta) is digamma(omega) - digamma(epsilon+omega)
        '''

        a, b = [], []
        a, b = np.append(a, self.alpha), np.append(a, self.beta)
        return digamma(np.array(a)) - digamma(np.array(a) + np.array(b))
...
class Xi(Beta):
    def __init__(self, *args, **kwargs):
        Beta.__init__(self, *args, **kwargs)


class S(Bernoulli):
    def __init__(self, *args, **kwargs):
        Bernoulli.__init__(self, *args, **kwargs)
...
def update_beta_gam(params):
    ...
    for n in range(N):
        for k in range(K):
            for j in range(J):
                val = (cells[n,:] ** 2) - 2 * cells[n,:] * add_noise(j) * theta_prev[n] + (add_noise(j) ** 2) * (tau_prev[n] + theta_prev[n] ** 2)
...
# SNV parts: variant_read_counts, p, sigma, T, B
def update_pi(delta, expected_hidden, theta, tau, alpha, beta, cells, p_X, sigma):
    K = delta.size
    J = len(expected_hidden[0, :, 0])
    ...
    updated_pi = pi._get_expectation_of_log() + sums
    
    # SNV
    for n in range(N):
        for k in range(K):
            for l in range(L):
                # If not missing (MCAR is assumed i.e. if missing count we just ignore that in the loop)
                if not math.isnan(p_X[n, l, 0]):
                    updated_pi[n, k] += sigma[k, l] * p_X[n, l, 0] # p_X is in log scale
                if not math.isnan(p_X[n, l, 1]):
                    updated_pi[n, k] += (1 - sigma[k, l]) * p_X[n, l, 1]
...
def calculate_expectation_of_D(j, theta_n, tau_n, alpha, beta, cell_nm):
    ...
    E_sigma_log_1sigma2 = gam._get_expectation_of_log()
...
def update_weight_edge(lam, pi):
    ...
    for i in range(J):
        for k in range(K):
            a = A(lam[k, i])
            weight_edge[k, i] = a._get_expectation_of_log()
...
def update_theta_tau(params):
    ...
    for k in range(K):
        for j in range(J):
            for m in range(M):
                sum += pi_prev[k] * expected_hidden[k, j, m] * (add_noise(j) ** 2)
                sum_2 += pi_prev[k] * expected_hidden[k, j, m] * add_noise(j) * cell[m]
...
class Prior(object):
    def __init__(self, delta, theta, tau, alpha_gam, beta_gam, lam, gamma, eta):
        self.delta = delta
        self.theta = theta
        self.tau = tau
...
class Params(object):
    def __init__(self, trans, delta, theta, tau, alpha_gam, beta_gam, lam, pi, weight_initial, weight_edge, weight_vertex, gamma, eta, sigma):
        self.delta = delta
        self.theta = theta
        self.tau = tau
        self.alpha_gam = alpha_gam
    ...
    def print(self):
        return (self.trans, self.delta, self.theta, self.tau, self.alpha_gam, self.beta_gam, self.lam, self.pi,
                 self.weight_initial, self.weight_edge, self.weight_vertex, self.gamma, self.eta, self.sigma)

    def get_term1_inner(self, params):
        pi, theta, tau, c, expected_hidden = params
        res = 0
        for m in range(self.M): #tqdm.tqdm(range(self.M)):
    ...
    def calculate_expectation_full_joint_probability(self, cells, prior, clusters):
        ...
        def get_term2():
            ...
            for k in range(self.K):
                for i in range(self.J):
                    a = A(self.lam[k, i])
                    term += np.sum(sum_of_expected_hidden_two[k, i, :] * a._get_expectation_of_log())
            return term
        ...
        def get_term3():
            ...
            pi = Pi(self.delta)
            for n in range(self.N):
                res += np.sum(self.pi[n, :] * pi._get_expectation_of_log())
            return res
        ...
        def get_term5():
            ...
            for k in range(self.K):
                for i in range(self.J):
                    a = A(self.lam[k, i])
                    term1 = np.sum((prior.lam[k, i, :] - 1) * a._get_expectation_of_log())
        ...
        def get_term7():
            pi = Pi(self.delta)
            term1 = np.sum((prior.delta - 1) * pi._get_expectation_of_log())
            return term1 #- math.log(beta_func(prior.delta) + .00000000001)
    ...
    def calculate_expectation_entropy(self, clusters):
        ...
        def get_term4():
            ...
            for k in range(self.K):
                for i in range(self.J):
                    a = A(self.lam[k, i])
                    term1 = np.sum((self.lam[k, i, :] - 1) * a._get_expectation_of_log())
                    res += term1  # - math.log(beta_func(self.lam[k, i]) + .00000000001)
            return res
        ...
        def get_term6():
            pi = Pi(self.delta)
            return np.sum(pi._get_expectation_of_log() * (self.delta - 1))
...
def update_eta_gamma(gamma, eta, sigma):
    K = sigma.shape[0]
    new_eta, new_gamma = np.zeros(K), np.zeros(K)
    for k in range(K):
        new_eta[k] = eta - 1 + np.sum(1 - sigma[k])
        new_gamma[k] = gamma - 1 + np.sum(sigma[k])
    return new_eta, new_gamma


def update_sigma(gamma, eta, p_X, pi):
    K = pi.shape[1]
    L = p_X.shape[1]
    N = pi.shape[0]
    log_normalized, updated_sigma, sum = np.zeros((K, L)), np.zeros((K, L)), np.zeros((K, L))
    ...
    for l in range(L):
        for k in range(K):
            for n in range(N):
                if not math.isnan(p_X[n, l, 0]):
                    sum[k, l] += pi[n,k] * p_X[n, l, 0] # p_X is in log scale

    for k in range(K):
        xi = Xi(gamma[k], eta[k])
        updated_sigma[k] = xi._get_expectation_of_log() + sum[k]

    for l in range(L):
        log_normalized[:, l] = updated_sigma[:, l] - logsumexp(updated_sigma[:, l])

    return np.exp(log_normalized)


# Requires numpy arrays (do np.array() before passing arguments)
def vi(prior, init, y, p_X, max_iter=25, tol=.01):
    N = len(y[:, 0])
    import multiprocessing as mp

    delta_prior, theta_prio, tau_prior, alpha_gam_prior, beta_gam_prior, lam_prior,  gamma_prior, eta_prior = prior
    delta, theta, tau, alpha_gam, beta_gam, lam, pi, weight_initial, weight_edge, weight_vertex, gamma, eta, sigma = init
    prior = Prior(delta_prior, theta_prio, tau_prior, alpha_gam_prior, beta_gam_prior, lam_prior, gamma_prior, eta_prior)
    ...
    init_params = Params(weight_edge, delta, theta, tau, new_alpha_gam, beta_gam, lam, pi, weight_initial, weight_edge, weight_vertex, gamma, eta, sigma)
    ...
    while m < max_iter:
        ...
        # Update 6
        pool = mp.Pool(200)
        result = pool.map(update_theta_tau, [(prior.theta, prior.tau, p_prev.pi[n], expected_hidden, p_prev.alpha_gam, p_prev.beta_gam, y[n]) for n in range(N)])
        ...
        # Update 8
        new_sigma = update_sigma(p_prev.gamma, p_prev.eta, p_X, p_prev.pi)# dummy np.ones((K, 10)) * .4

        # Update 9
        new_eta, new_gamma = update_eta_gamma(prior.gamma, prior.eta, new_sigma)

        # Update 10
        new_pi = update_pi(new_delta, expected_hidden, new_theta, new_tau, new_alpha_gam, new_beta_gam, y, p_X, new_sigma)
        ...
        p_next = Params(trans, new_delta, new_theta, new_tau, new_alpha_gam, new_beta_gam, new_lam, new_pi,
                        weight_initial, new_weight_edge, new_weight_vertex, new_gamma, new_eta, new_sigma)
